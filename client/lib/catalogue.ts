...
const data = bundledData as { catalogueVersion: string; activities: Activity[] };
...
export async function refreshFromNetwork(): Promise<{
  ...
  if (!baseUrl) {
    return { ok: false, message: "No server configured. Using saved catalogue." };
  }
  try {
    const response = await fetch(new URL("/api/activities", baseUrl).href);
    if (!response.ok) {
      return { ok: false, message: "Server not reachable. Using saved catalogue." };
    }
    ...
    return { ok: false, message: "Could not reach server. Using saved catalogue." };
  }
}
